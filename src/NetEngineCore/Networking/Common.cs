    ...
    public abstract class Common {
        ...
        protected static bool SendMessagesBlocking(Stream stream, byte[][] messages) {
            ...
            try {
                ...
                logger.Info("Send: stream.Write exception: " + exception);
                return false;
            }
        }
        ...
        protected static void ReceiveLoop(int connectionId, TcpClient client, ConcurrentQueue<Packet> receiveQueue,
            ...
            try {
                ...
                // it's needed
                receiveQueue.Enqueue(new Packet(connectionId, PacketType.Connection, null));
                ...
                logger.Info("ReceiveLoop: finished receive function for connectionId=" + connectionId + " reason: " +
                            exception);
            }
            ...
            //    would be closed.
            receiveQueue.Enqueue(new Packet(connectionId, PacketType.Disconnection, null));
        }
